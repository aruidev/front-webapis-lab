 ...
 */
async function entrar(target: Element = document.documentElement): Promise<void> {
 ...
 */
async function alternar(target: Element = document.documentElement): Promise<void> {
  if (esActiu()) {
    await sortir();
  } else {
    await entrar(target);
 ...
 */
function enCanvi(handler: (el: Element | null) => void) {
 ...
 */
function enError(handler: (ev: Event) => void) {
 ...
 */
function sincronitzarTextBoto(btn: HTMLElement, labels = { enter: "Go Fullscreen", exit: "Exit Fullscreen" }) {
 ...
 */
function vincularBotoAlternar(btn: HTMLElement, target: Element = document.documentElement, labels?: { enter: string; exit: string }) {
  // Inicializa texto
