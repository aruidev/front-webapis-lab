...
type BarOptions = {
	alturaMax?: number; // Altura máxima en px, si no se indica usa la altura del contenedor.
	delayMs?: number; // Retardo entre barras (default 500ms).
	duracionMs?: number; // Duración total de la animación (default 2000ms).
	easing?: (t: number) => number; // Easing opcional (t normalizado 0-1).
	onFin?: () => void; // Callback al terminar la animación.
};
 ...
 */
function crearGraficoBarras(container: HTMLElement, datos: BarDatum[], options: BarOptions = {}): BarChartApi {
	...
	let startTime = 0;
	let currentDatos: BarDatum[] = [...datos];
     ...
     */
	const crearBarra = (dato: BarDatum) => {
     ...
     */
	const setDatos = (datosNuevos: BarDatum[]) => {
