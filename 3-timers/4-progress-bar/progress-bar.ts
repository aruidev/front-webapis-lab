type ProgressOptions = {
	initial?: number; // Valor inicial (0-100)
	heightPx?: number; // Altura de la barra en px
	background?: string; // Color/fondo de la pista
	fill?: string; // Color/fondo del relleno
	animationMs?: number; // Duración por defecto de la animación
	easing?: (t: number) => number; // Easing opcional
	showLabel?: boolean; // Mostrar texto de porcentaje
	formatLabel?: (value: number) => string; // Formateador opcional de etiqueta
};

type ProgressBarApi = {
	set: (valor: number, animate?: boolean, customDurationMs?: number) => void;
	get: () => number;
	reset: () => void;
	destroy: () => void;
};
 ...
 */
function crearBarraProgreso(container: HTMLElement, options: ProgressOptions = {}): ProgressBarApi {
	...
	if (!options.showLabel) label.style.display = "none";
	...
	const render = (value: number) => {
		...
		if (options.showLabel) label.textContent = formatLabel(pct);
	};
     ...
     */
	const set = (valor: number, animate = true, customDurationMs?: number) => {
		if (destroyed) return;
