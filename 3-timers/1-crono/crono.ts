type CronoState = "stop" | "pause" | "run";

type CronoOptions = {
	tickMs?: number; // Frecuencia de refresco en ms (por defecto 100ms para décimas)
	onTick?: (ms: number, formatted: string) => void;
	onStateChange?: (state: CronoState) => void;
	onMarca?: (ms: number, formatted: string, marcas: string[]) => void;
};
 ...
 */
function crearCronometro(options: CronoOptions = {}): CronoApi {
	const { tickMs = 100, onTick, onStateChange, onMarca } = options;

	let estado: CronoState = "stop";
     ...
     */
	const setEstado = (nuevo: CronoState) => {
		if (estado !== nuevo) {
			estado = nuevo;
