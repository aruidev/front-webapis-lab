...
import type { Atributs } from "./elements.js";
 ...
 */
function esborrarFiles(taula: HTMLTableElement, n = 1, posicio = 0) {
 ...
 * @param posicio - Posición inicial (0-based).
 */
function esborrarColumnes(taula: HTMLTableElement, n = 1, posicio = 0) {
	const start = Math.max(posicio, 0);
 ...
 */
function omplirCasella(taula: HTMLTableElement, fila: number, columna: number, contingut: string) {
 ...
 */
function omplir(taula: HTMLTableElement, dades: any[][]) {
 ...
 */
function crearTaula(files: number, columnes: number, atributs: Atributs = {}, dades: any[][] = []) {
	...
	return taula as HTMLTableElement;
}
 ...
 * @param taula - Tabla objetivo.
 */
function MoureColumnesAnterior(taula: HTMLTableElement) {
    for (const row of Array.from(taula.rows)) {
 ...
 * @param taula - Tabla objetivo.
 */
function MoureColumnesSeguent(taula: HTMLTableElement) {
    for (const row of Array.from(taula.rows)) {
        if (row.cells.length > 0) {
